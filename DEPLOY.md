...
**שים לב:** 86 קבצי התמונות יושבים בשורש הריפו. הם **לא** נפרסים —
`.vercelignore` מכסה אותם (`/*.jpg`, `/*.jpeg`, `/*.png`, `/*.heic`,
`/תמונות`; הסלאש בתחילת השורה מקבע לשורש, כך שהנגזרות ב-`assets/img/` נשארות).

אבל git עדיין עוקב אחריהם, ו-`.gitignore` לא משפיע על קובץ שכבר במעקב. פעם
אחת, מהמכונה שממנה דוחפים:

```bash
git rm --cached --ignore-unmatch -- \
  ':(top,glob)*.jpg' ':(top,glob)*.jpeg' ':(top,glob)*.png' \
  ':(top,glob)*.heic' ':(top)תמונות'
git commit -m "untrack the raw photo library"
```

`--cached` מסיר מהאינדקס בלבד — 86 הקבצים נשארים על הדיסק, וזה הכרחי:
`tools/manifest.json` ממפה כל סלוט לשם קובץ בשורש. הפטרנים ב-`.gitignore`
מונעים שיחזרו. עץ ה-HEAD יורד מ-41.6MB ל-9.5MB ו-`git clone --depth 1`
מ-31MB ל-7MB. ההיסטוריה לא נכתבת מחדש, אז שכפול מלא עדיין מוריד את הבלובים
הישנים — זה בכוונה, כתיבה מחדש של היסטוריה משותפת גרועה יותר מ-30MB.
