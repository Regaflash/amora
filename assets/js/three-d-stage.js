...
(() => {
  ...
  class ThreeDStage extends HTMLElement {
    constructor() {
      ...
      if (!this.hasAttribute('hide-toolbar')) root.appendChild(this._toolbar);
    ...
    connectedCallback() {
      ...
      this._boot().catch((err) => {
        ...
        if (fallback) {
          // Degrade to a still image — the section still looks intentional.
          const img = document.createElement('img');
          img.src = fallback;
          img.alt = this.getAttribute('fallback-alt') || '';
          img.style.cssText = 'width:100%;height:100%;object-fit:contain;display:block';
          this.shadowRoot.insertBefore(img, this._err);
          this._toolbar.remove();
          const n = this.shadowRoot.querySelector('.note');
          if (n) n.remove();
        } else {
          this._err.style.display = 'flex';
          this._err.textContent = this.getAttribute('error-text') ||
            'three.js failed to load. Check the pinned import map in <head>.';
        }
        console.error('three-d-stage:', err);
